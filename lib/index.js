...
const GrudgesListItem = ({ id, jackalName, offense, active, forgiven }) => {
  return (
    <div className={active ? 'GrudgesListItem is-active' : 'GrudgesListItem'}>
      <h3 className="GrudgesListItem-jackalName">{jackalName}
        {forgiven ? ' (Forgiven)' : (' (UNFORGIVEN)')}</h3>
...
const GrudgeCount = () => {
  return (
    <div>
      <p>Serving up {store.all().length} total grudges.</p>
      <p>UNFORGIVEN: {store.unforgiven()}</p>
      <p>Forgiven: {store.forgiven()}</p>
    </div>
  );
};
